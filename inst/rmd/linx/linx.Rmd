...
params <- list(
  sample = "SBJ01229_MDX210456_L2101648",
  table_dir = here::here("nogit/linx/SBJ01229/tables"),
  plot_dir = here::here("nogit/linx/SBJ01229/plots")
)
render_me <- function() {
  rmarkdown::render(
    here::here("inst/rmd/linx/linx.Rmd"),
    params = params,
    output_file = "SBJ001229.html",
...
linx_path_plot_chr <- function(chr) {
  ...
  if (length(x) == 0) {
    warning(glue::glue("No LINX plots available for chromosome {chr}."))
    return(NULL)
  } else if (length(x) > 1) {
    stop(glue::glue("Multiple LINX plots found for chromosome {chr}. Aborting."))
  }
