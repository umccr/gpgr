...
apgi <- readRDS(file = here::here("nogit/inputs/apgi/maf/all_merged.rds"))
```
...
### Fit COSMIC reference signatures

* **SBS:**

```{r}
ind <- apply(apgi_SBS$nmf_matrix, 1, sum) %>% which.max()
# ind <- apply(laml_SBS$nmf_matrix, 1, sum) %>% which.max()
sample1_sbs <- apgi_SBS$nmf_matrix[ind, , drop = FALSE] %>% t()
# sample1 <- laml_SBS$nmf_matrix[ind, , drop = FALSE] %>% t()
head(sample1_sbs)
sigminer::sig_fit(sample1_sbs, sig_index = 1:30)
sigminer::sig_fit(sample1_sbs, sig_index = 1:30, type = "relative")
sigminer::sig_fit(t(apgi_SBS$nmf_matrix[1:5,]), sig_index = 1:30, rel_threshold = 0.05)
```
* **DBS:**

```{r}
ind <- apply(apgi_DBS$nmf_matrix, 1, sum) %>% which.max()
sample1_dbs <- apgi_DBS$nmf_matrix[ind, , drop = FALSE] %>% t()
sigminer::sig_fit(sample1_dbs, sig_index = 1:11, sig_db = "DBS")
```

### Fit custom signatures

```{r}
sig_fit(sample1, sig = sigs)
```
...
```{r}
bt_result <- sig_fit_bootstrap(sample1, sig = sigs, n = 10)
```
...
```{r}
show_catalogue(t(apgi_SBS$nmf_matrix), style = "cosmic", x_label_angle = 90)
show_catalogue(t(apgi_SBS$nmf_matrix), style = "cosmic", x_label_angle = 90,
               samples = c("APGI_1151_tumor", "APGI_1830_tumor"))
```
