...
## Run CHORD
The samples used for CHORD HRD predictions are shown below.
...
* Run with direct SV VCF and SV vcf2df inputs:
...
```{r chord_results}
res_df <- readRDS(here::here("nogit/chord/rds/patients/chord_results_2020-09-28_dfinput.rds"))
res_vcf <- readRDS(here::here("nogit/chord/rds/patients/chord_results_2020-09-27_vcfinput.rds"))
p_df <- res_df %>%
...
## Display CHORD Results {.tabset .tabset-fade #signatures}

### data.frame input

```{r chord_tables1}
p_df %>%
  filter(sample != "PM3056445") %>% # this is duplicated so just remove for now
  dplyr::left_join(
    s %>% dplyr::select(sample = Alias, Prefix),
    by = "sample"
  ) %>%
  DT::datatable(filter = list(position = "top", clear = FALSE, plain = TRUE),
                caption = "CHORD HRD predictions.",
                class = "cell-border display compact",
                rownames = FALSE, extensions = c("Scroller", "Buttons", "KeyTable"),
                options = list(
                  scroller = TRUE, scrollY = 2000, scrollX = TRUE, autoWidth = FALSE, keys = TRUE,
