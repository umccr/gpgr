...
write_tsvgz <- function(x, path, ...) {
  ...
  readr::write_tsv(x = x, path = file.path(result_outdir, path), ...)
}

write_jsongz <- function(x, path, ...) {
  assertthat::assert_that(endsWith(path, ".gz"))
  jsonlite::write_json(x = x, path = gzfile(file.path(result_outdir, "json",  path), open = "w"), ...)
}
...
write_tsvjsongz(qc_summary_all, "qc_summary")
