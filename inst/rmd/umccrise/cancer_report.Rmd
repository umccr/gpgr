...
dragen_hrd_res <- gpgr::dragen_hrd()
...
# Extract PCGR status fields written by bolt (sash #52)
...
#   "tier N / noncoding / other: 80,321 variants"
pcgr_format_categories <- function(cats_str) {
  ...
  lines <- vapply(entries, function(e) {
    ...
    tl <- tier_labels[[tier]]   %||% tier
    il <- impact_labels[[impact]] %||% impact
    rl <- region_labels[[region]] %||% region
    glue::glue("- {tl} / {il} / {rl}: {count} variants")
  }, character(1), USE.NAMES = FALSE)
  paste(lines, collapse = "\n")
}

# Short label appended to snv_summary value
pcgr_value_fragment <- ifelse(
  pcgr_skipped, "PCGR skipped",
  ifelse(pcgr_selection_applied,
    glue::glue("PCGR filtered to {format(snv_summary_counts$pcgr_selection_output, big.mark = ',', trim = TRUE)}"),
    "")
)
# Detailed breakdown shown in the details column of the SNVs row
pcgr_details <- ifelse(
  pcgr_skipped,
  glue::glue("PCGR skipped: could not reduce variants below the 500K limit ({snv_summary_counts$pcgr_skip_reason})"),
  ifelse(
    pcgr_selection_applied,
    glue::glue(
