...
if (is.null(params$chord_file)) {
  ...
  prediction = convert_json_gz_to_tibble(params$chord_file)
    )
}
...
if (is.null(params$mutpat_dir)) {
  ...
  sigs_snv_2015 <- gpgr::convert_json_gz_to_tibble(file.path(params$mutpat_dir, glue::glue("snv2015.json.gz")))%>%
  dplyr::select(Rank, Signature, Contribution, RelFreq)

  sigs_snv_2020 <- gpgr::convert_json_gz_to_tibble(file.path(params$mutpat_dir, glue::glue("snv2020.json.gz")))%>%
  dplyr::select(Rank, Signature, Contribution, RelFreq)

  sigs_dbs     <- gpgr::convert_json_gz_to_tibble(file.path(params$mutpat_dir, glue::glue("dbs.json.gz")))

  sigs_indel    <- gpgr::convert_json_gz_to_tibble(file.path(params$mutpat_dir, glue::glue("indel.json.gz")))
...
p1 <- gpgr:::plot_bnd_sr_pr_tot_lines(sv$map)
p2 <- gpgr::plot_bnd_sr_pr_tot_hist(sv$map) + ggplot2::theme(legend.position = "none")
