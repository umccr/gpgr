...
# Parse "tier|impact|region:count, ..." into readable lines, e.g.
#   "tier N / noncoding / other: 80,321 variants". Malformed entries (missing
# a '|' or ':' field) are skipped rather than erroring, since this feeds a
# clinical report and a single bad category shouldn't abort the whole render.
...
pcgr_details <- switch(pcgr_state,
  ...
  selection_applied = glue::glue(
    ...
    "Dropped categories:\n{pcgr_format_categories(unlist(snv_summary_counts$pcgr_selection_filters) %||% '')}"
  ),
  none = ""
)
