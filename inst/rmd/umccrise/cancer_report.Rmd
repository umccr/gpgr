...
dragen_available <- !is.null(params$dragen_hrd)
...
# Extract PCGR status fields written by bolt (sash #52). pcgr_skipped and
# pcgr_selection_applied are mutually exclusive by construction on the bolt
# side (a sample is either skipped, selection-applied, or neither).
pcgr_skipped <- isTRUE(snv_summary_counts$pcgr_skipped)
pcgr_selection_applied <- isTRUE(snv_summary_counts$pcgr_selection_applied)
pcgr_state <- dplyr::case_when(
  pcgr_skipped ~ "skipped",
  pcgr_selection_applied ~ "selection_applied",
  TRUE ~ "none"
)

# pcgr_format_categories() parses "tier|impact|region:count, ..." into
# readable lines, e.g. "tier N / noncoding / other: 80,321 variants".
# Malformed entries (missing a '|' or ':' field) are skipped rather than
# erroring, since this feeds a clinical report and a single bad category
# shouldn't abort the whole render. Defined in R/umccrise.R (exported by
# gpgr, loaded above) so it has real test coverage instead of living only
# inside this Rmd chunk.

# Short label appended to snv_summary value, and the detailed breakdown shown
# in the SNVs row's details column — both derived from the single pcgr_state
# above so the two can't drift out of sync with each other.
pcgr_value_fragment <- switch(pcgr_state,
  skipped = "PCGR skipped",
  selection_applied = glue::glue(
    "PCGR filtered to {format(snv_summary_counts$pcgr_selection_output, big.mark = ',', trim = TRUE)}"
  ),
  none = ""
)
pcgr_details <- switch(pcgr_state,
  skipped = glue::glue(
    "PCGR skipped: could not reduce variants below the 500K limit ({snv_summary_counts$pcgr_skip_reason})"
  ),
  selection_applied = glue::glue(
    "PCGR 500K limit applied: {format(snv_summary_counts$pcgr_selection_filtered, big.mark = ',', trim = TRUE)} ",
    "variants dropped from {format(snv_summary_counts$pcgr_selection_output + snv_summary_counts$pcgr_selection_filtered, big.mark = ',', trim = TRUE)} ",
    "to {format(snv_summary_counts$pcgr_selection_output, big.mark = ',', trim = TRUE)}.\n",
    "Dropped categories:\n{gpgr::pcgr_format_categories(rlang::`%||%`(unlist(snv_summary_counts$pcgr_selection_filters), ''))}"
  ),
  none = ""
)
hypermutated_details <- ifelse(
  !hypermutated, "",
  "More than 500K SNVs detected (including non-PASS), priority regions and/or variants selected for processing."
)

# Exclude bookkeeping keys from the SNV counts string (like is_hypermutated)
pcgr_keys <- grep("^pcgr_", names(snv_summary_counts), value = TRUE)
snv_summary <- snv_summary_counts[!names(snv_summary_counts) %in% c("is_hypermutated", pcgr_keys)] |>
  unlist() |>
  tibble::enframe() |>
  dplyr::mutate(
  ...
  dplyr::pull(x) |>
  paste(collapse = "; ")
if (nzchar(pcgr_value_fragment)) {
  snv_summary <- paste0(snv_summary, "; ", pcgr_value_fragment)
}

hrd_summary <- list(
  dragen = if (dragen_available) {
...
hrd_row_text <- if (dragen_available) {
  glue::glue("DRAGEN: {hrd_summary$dragen}; CHORD: {hrd_summary$chord}; HRDetect: {hrd_summary$hrdetect}")
} else {
  glue::glue("CHORD: {hrd_summary$chord}; HRDetect: {hrd_summary$hrdetect}")
}
hrd_row_details <- if (dragen_available) {
  "Homologous recombination deficiency score (DRAGEN) / probability (CHORD, HRDetect)."
} else {
  "Homologous recombination deficiency probability (CHORD, HRDetect)."
}
...
qc_summary_all <- dplyr::tribble(
  ...
  4, "Hypermutated", ifelse(hypermutated, "TRUE", "FALSE"), hypermutated_details,
  ...
  6, "HRD", hrd_row_text, hrd_row_details,
  20, "CNVs (Somatic)", glue::glue(
    "Min: {cnv_summary[['som']]$Min}; ",
    "Max: {cnv_summary[['som']]$Max}; ",
    "N: {cnv_summary[['som']]$N} "
  ), "",
  17, "SNVs (Somatic)", snv_summary, paste0("Summary SNV counts across main processes, in order.", ifelse(nzchar(pcgr_details), paste0("\n", pcgr_details), "")),
...
```{r dragen_description, results='asis', echo=FALSE}
if (dragen_available) {
  cat(
    "__DRAGEN__\n\n",
  ...
  cat("__DRAGEN__: *not available for this sample — no DRAGEN output was provided (e.g. OA-only mode).*\n")
}
```
...
hrd_results <- gpgr::hrd_results_tabs(
  hrdetect_res, chord_res,
  dragen_res = if (dragen_available) dragen_hrd_res else NULL
)
