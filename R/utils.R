...
#' Read a data frame from gzipped TSV and JSON files
#'
#' Reads a data frame from gzipped TSV and JSON files. Files
#' are expected to be found at `maindir/<path>.tsv.gz` and
#' `maindir/json/<path>.json.gz`.
#'
#' @param path Relative path to read the files from, sans the file extensions -
#' these will be appended appropriately.
#' @param maindir Main directory to read the files from.
#' @param ... Additional arguments passed to the reading functions.
#'
#' @return A list with two elements:
#' \describe{
#'   \item{tsv}{The data frame read from the TSV file.}
#'   \item{json}{The data frame read from the JSON file.}
#' }
#'
#' @export
read_tsvjsongz <- function(path, maindir, ...) {
  tsv_path <- file.path(maindir, paste0(path, ".tsv.gz"))
  json_path <- file.path(maindir, "json", paste0(path, ".json.gz"))

  # Read the gzipped TSV file (readr automatically handles gzip files)
  tsv_data <- readr::read_tsv(tsv_path, ...)

  # Read the gzipped JSON file
  json_data <- jsonlite::read_json(json_path, simplifyVector = TRUE, ...)

  list(tsv = tsv_data, json = json_data)
}

#' Read a data frame from a gzipped TSV file
#'
#' Reads a data frame from a gzipped TSV file. The file is expected to be
#' located at `maindir/<path>.tsv.gz`.
#'
#' @param path Relative path to the file (without the file extension).
#' @param maindir Main directory where the file is stored.
#' @param ... Additional arguments passed to [readr::read_tsv()].
#'
#' @return A data frame read from the gzipped TSV file.
#'
#' @export
read_tsvgz <- function(path, maindir, ...) {
  tsv_path <- file.path(maindir, paste0(path, ".tsv.gz"))
  readr::read_tsv(tsv_path, ...)
}

#' Read a data frame from a gzipped JSON file
#'
#' Reads a data frame from a gzipped JSON file. The file is expected to be
#' located at \code{maindir/<path>.json.gz}.
#'
#' @param path Relative path to the file (without the file extension).
#' @param maindir Main directory where the file is stored.
#' @param ... Additional arguments passed to \code{\link[jsonlite]{read_json}}.
#'
#' @return A data frame (or tibble) read from the gzipped JSON file.
#'
#' @export
read_jsongz <- function(path, maindir, ...) {
  json_path <- file.path(maindir, paste0(path, ".json.gz"))
  con <- gzfile(json_path, open = "rt")
  data <- jsonlite::read_json(con, simplifyVector = TRUE, ...)
  close(con)
  return(data)
}
